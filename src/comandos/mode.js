  ...
  async (msg, textoMensagem, servidor) => {
    ...
    if (!novoModo) {
      util.sendEmbed(
        msg,
        locale,
        usoIncorretoDoComando(servidor.locale),
        locale.usoMode(servidor.locale, { prefixo }),
        false
      );
      return;
    }
    ...
    if (modos.indexOf(novoModo) === -1) {
      util.sendEmbed(
        ...
        locale.modosExistentes(servidor.locale, { modos }),
        false
      );
      return;
    }
    ...
    const camposAlterados = {
      somente_perguntas_servidor: false,
      somente_perguntas_globais: false,
    };
    if (novoModo === 'server') {
    ...
    if (resultado.sucesso) {
      util.sendEmbed(
        msg,
        locale.modoAtualizado(servidor.locale),
        locale.mensagemModoAtualizado(servidor.locale, { novoModo })
      );
    } else {
      util.sendEmbed(
        msg,
