...
const changeLanguage = new Comando(
    ...
    async (msg, textoMensagem, servidor) => {
        ...
        if (localesValidos.indexOf(novoLocale) === -1) {
            util.sendEmbed(msg, locale.modoInvalido(servidor.locale), locale.localesExistentes(servidor.locale, { localesValidos }), false);
            return;
        }
        ...
        if (resultado.sucesso) {
            util.sendEmbed(msg, locale.localeAtualizado(novoLocale), locale.mensagemLocaleAtualizado(novoLocale, { novoLocale }));
        } else {
