...
const newQuestion = new Comando(
  ...
  async (msg, textoMensagem, servidor) => {
    const primeiraOpcao = /(?<=^1\s*-).+(?=2\s*-)/g.exec(textoMensagem);
    const segundaOpcao = /(?<=2\s*-).+/g.exec(textoMensagem);

    if (!primeiraOpcao || !segundaOpcao) {
      util.sendEmbed(
        msg,
        locale.usoIncorretoDoComando(servidor.locale),
        locale.usoNewQuestion(servidor.locale, { prefixo }),
        false
      );
      return;
    }

    const resposta = await PerguntaService.create(
      primeiraOpcao[0].trim(),
      segundaOpcao[0].trim(),
      servidor.id_servidor,
      servidor.locale
    );
    util.sendResultEmbed(
      msg,
      resposta.mensagem(servidor.locale, { id: resposta.id }),
      servidor,
      resposta.sucesso
    );
  },
