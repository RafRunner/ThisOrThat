...
function montaMensagemPerguntas(titulo, perguntas, servidor) {
  const reducer = (acumulador, pergunta) =>
    acumulador + locale.acumuladorPergunta(servidor.locale, { pergunta });
...
const listQuestions = new Comando(
  ...
  async (msg, textoMensagem, servidor) => {
    ...
    if (resposta.paginas === 0) {
      util.sendEmbed(
        msg,
        locale.servidorSemPerguntas(servidor.locale),
        locale.cadastreNovasPerguntas(servidor.locale),
        false
      );
      return;
    }
    ...
    const collector = mensagemPerguntas.createReactionCollector(filter, {
      time: 300 * 1000,
      max: 1000,
      dispose: true,
    });
    ...
    const mudarPagina = async (reaction, user) => {
      ...
      if (novaResposta.sucesso) {
        mensagemPerguntas.edit({
          embeds: [
            montaMensagemPerguntas(
              locale.listagemPaginas(servidor.locale, { page, resposta }),
              novaResposta.perguntas,
              servidor
            ),
          ],
        });
