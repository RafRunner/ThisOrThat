...
const listQuestions = new Comando(
    ...
    async (msg, textoMensagem, servidor) => {
        ...
        if (resposta.paginas === 0) {
            util.sendEmbed(msg, locale.servidorSemPerguntas(servidor.locale), locale.cadastreNovasPerguntas(servidor.locale), false);
            return;
        }
        ...
        const mudarPagina = async (reaction, user) => {
            ...
            if (novaResposta.sucesso) {
                mensagemPerguntas.edit({
                    embeds: [montaMensagemPerguntas(locale.listagemPaginas(servidor.locale, { page, resposta }), novaResposta.perguntas, servidor)],
                });
            }
        };
