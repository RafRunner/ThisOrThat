  ...
  async (msg, textoMensagem, servidor) => {
    ...
    if (!novoTempo) {
      util.sendEmbed(
        ...
        locale.usoTimeout(servidor.locale, { prefixo }),
        false
      );
      return;
    }

    const resultado = await ServidorService.update(servidor.id_servidor, {
      tempo_para_responder: novoTempo[0],
    });
    if (resultado.sucesso) {
      util.sendEmbed(
        msg,
        locale.tempoAtualizado(servidor.locale),
        locale.mensagemTempoAtualizado(servidor.locale, { novoTempo })
      );
    } else {
      util.sendEmbed(
        msg,
